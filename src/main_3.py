...
def compute_fifo_delay_modular(flows, edges, global_overhead):
    ...
    for hop in range(max_hops):
        # For each edge, check if any flows arrive this hop
        for edge, total_flows in edge_to_flows.items():
            ...
            for _, _, key, _, _ in arrivals:
                edge_arrivals[edge].add(key)
            # Only process if ALL possible flows have arrived at this edge
            if edge_arrivals[edge] == total_flows and total_flows:
                # "Wait" until all flows are present
                burst_sum = compute_burst_sum(burst_map, arrivals, path_map, hop)
                capacity = get_edge_capacity(edges, edge)
                delay = burst_sum / capacity
                for flow, target, key, path, hop_idx in arrivals:
                ...
                edge_arrivals[edge] = set()  # Reset for next period
...
# burst_table.clear()
stations, switches, edges, flows = parse_afdx_xml("data/check_2.xml")
global_overhead = 67
